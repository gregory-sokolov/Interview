...
{
public:
	/// Simple method to ñalculate bit count of a number.
	...
	/// Optimized method to ñalculate bit count of a number (by Brian Kernighan).
	...
	static unsigned short GetBitCountTbl(long long value, const vector<unsigned short>& table)
	{
		unsigned short result = 0;
	...
	static void GenerateWordTable(vector<unsigned short>& table)
	{
	...
	static vector<vector<string>> GenereatePowerSet(const vector<string>& input)
	{
	...
	static vector<unsigned short> GetSetBitIndexes(unsigned long long value)
	{
		vector<unsigned short> results;
		auto shifted = value;
